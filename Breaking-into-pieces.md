...
- [Main](#main)
  ...
  - [Sorting Mirrorlist](#sorting-mirrorlist)
...
declare DISK_CHECK=""    #Function variable
declare PART_CHECK=""    #Function variable
declare NUMBER_CHECK=""  #Function variable
declare PKG_SELECT=""    #Function variable
declare PKG_FIND=""      #Function variable
declare ANSWER=""        #Function variable (yes_no)
declare SELECTION=""     #Function variable (select_one)
...
declare PKG_SPECIFIC_OPERATIONS="virtualbox clamav lightdm-slick-greeter lightdm-gtk-greeter" #Function variable
```

## Packages

The package sets that start with `ADDITIONAL_` prefix are optional packages. Each of them are parsed within the `pkg-select` [function](#pkg_select) and asked to user.
...
Transmit only one packet of data to `wiki.archlinux.org`. If command exits with an error, invoke the `failure` [function](#failure).

*Differs from other `check` functions.* (see: [Check functions](#check-functions))
...
Print the input string in different formats.
...
If the `'$DISK'` is defined, perform a set of actions:

- Unmount all of the partitions.

- Swapoff if there are active swap partitions.

- Close all of the logical volumes and LUKS partitions.
...
Remove the program lock and exit accordingly.
...
Accept only 'y' & 'n' (case insensitive).
...
#If user entered wrongly loop until the correct answer has gotten.
...
Takes an additional package set as an argument and asks each package to user. If user accepts, selected packages will be included in the original set.
...
Similar to `pkg_select` [function](#pkg_select), this one differs by selecting only one package. It is intended to be used in 'driver selection' kind of tasks. (usage: [Greeter](#greeter) - [Video Driver](#video-driver))
...
#i.e. syntax should be: SET+=" PACKAGE"
...
function post-install () {
    ...
    #Export variables to use it in the user's terminal
    export USER_NAME="$USER_NAME"
    
    #Export functions to call it in the user's terminal
    export -f clone_yay
...
For additional information about partition tables, visit this link <https://wiki.gentoo.org/wiki/Handbook:AMD64/Installation/Disks#Partition_tables>.
...
For additional information about trimming, visit this link <https://wiki.archlinux.org/title/Solid_state_drive#TRIM>.
...
if echo "$PARTED_INFO" | head -1 | grep -w -q SSD; then
    ...
    #Check if device supports trimming
...
Encryption is done according to: <https://wiki.archlinux.org/title/Dm-crypt/Encrypting_an_entire_system#LVM_on_LUKS>.
...
#Nano is a dependency for the script to work
